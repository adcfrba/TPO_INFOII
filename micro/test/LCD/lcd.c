...
void LCD_Display( uint8_t *msg , uint8_t r , uint8_t p )
{
	...
	if( r)
        p = p + 0xc0 ;//si vale cero aparece en la primera Fila
	else
		p = p + 0x80 ;//si vale uno aparece en la segunda Fila

	LCD_Escribir( p , LCD16x2_CONTROL );
	//p sirve para especificar en que posicion va ir el cartel, ejemplo p = 3 desde el inicio cuenta 3 cuadrados y luego pone el cartel 
	for( i = 0 ; msg[ i ] != '\0' ; i++ )
